...
* @ 换机器
	1、MAC可打在账号的GameInfo中，换机器须替换GameInfo里的MAC
	2、并将数据库中的存档数据迁移到新MAC
...
const kDBTableName2 = "SaveEncrypt"
...
// -------------------------------------
// -- Rpc
func Rpc_save_get_change_times(req, ack *common.NetPack) {
	mac := req.ReadString()
...
func Rpc_save_upload_data(req, ack *common.NetPack) {
	...
	if ok {
		dbmgo.UpdateToDB(kDBTableName2, bson.M{"_id": self.MAC}, bson.M{"$set": bson.M{
			"changetimes": self.ChangeTimes,
			"publickey":   self.PublicKey,
			"jsondata":    self.JsonData}})
	} else {
		dbmgo.InsertToDB(kDBTableName2, self)
	}
}
func Rpc_save_download_data(req, ack *common.NetPack) {
	...
	self.PublicKey = int32(math.RandBetween(0xff, 0xfffffff))
	dbmgo.UpdateToDB(kDBTableName2, bson.M{"_id": self.MAC}, bson.M{"$set": bson.M{"publickey": self.PublicKey}})
...
func LoadSaveDataFromDB(mac string) (*TSaveClient, bool) {
	...
	ok := dbmgo.Find(kDBTableName2, "_id", mac, data)
	return data, ok
}
