...
// ------------------------------------------------------------
// -- 玩家战斗数据
type TBattleInfo struct {
	AccountId uint32
	Name      string
	Head      string
}

func (self *TBattleInfo) BufToData(buf *common.NetPack) {
	self.AccountId = buf.ReadUInt32()
	self.Name = buf.ReadString()
	self.Head = buf.ReadString()
}
func (self *TBattleInfo) DataToBuf(buf *common.NetPack) {
	buf.WriteUInt32(self.AccountId)
	buf.WriteString(self.Name)
	buf.WriteString(self.Head)
}
func (self *TPlayer) GetBattleInfo() (ret TBattleInfo) {
	ret.AccountId = self.AccountID
	ret.Name = self.Name
	ret.Head = self.Head
	return
